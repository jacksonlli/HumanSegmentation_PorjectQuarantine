...
from image_transforms import get_trimaps, filter_masks, get_foreground_by_mask
import os

def inital_setup(images_path, raw_path):
    input_paths = get_input_paths()
    assert len(input_paths)>0
    clear_folder(images_path)
    copy_files(input_paths, images_path)
    clear_folder(raw_path)
    return create_output_dir(os.path.dirname(input_paths[0]))
...
#output_path = inital_setup(images_path, raw_path)
...
#mattes = alpha_matting_sequence(mattes, images_path, 80, isSaveTrimap=isSaveMasks, trimap_name="trimap2")
##--get final people segmentations
#segs = get_foreground_by_mask(mattes, images_path)
##--write pngs
#save_seg(segs, output_path)
